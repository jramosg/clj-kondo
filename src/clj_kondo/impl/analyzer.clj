...
(defn extract-map-bindings
  ...
  (let [;; in a namespaced map the reader qualifies :as, :or, :select,
    ...
    (let [key-bindings (volatile! [])
          or-key-nodes (when or?
                         (into []
                               (comp
                                (filter
                                 (fn [[k _]] (plain-directive? k :or)))
                                (mapcat
                                 (fn [[_ v]]
                                          ...
                                          (let [dk (when-not (identical? :clj-kondo/unknown-namespace modifier-ns)
                                                ...
                                                child-opts (if-let [vt (when dk
                                                                         (types/destructured-key-tag
                                                                          form-tag dk (defaulted? bname dk)))]
                                                             (assoc opts :tag vt)
                                                             opts)
    ...
    (with-meta parsed-bodies
      (when arities
        (cond-> {:arity {:fixed-arities fixed-arities
          ...
          pending-infer (assoc :pending-infer pending-infer))))))
...
(defn analyze-binding-call [ctx fn-name binding expr]
  (let [callstack (:callstack ctx)
    ...
    ;; calling a local fn is proof it runs here: commit its dormant
    ;; constraints on enclosing params, subject to reachability
    (when-let [pending (:pending-infer binding-info)]
...
(defn analyze-known-call
  ...
  (case resolved-as-clojure-var-name
    ...
    ;; catch-all
    (analyze-known-ns-call ctx expr children resolved-as-namespace resolved-as-name lint-as? resolved-namespace resolved-name defined-by defined-by->lint-as unresolved?)))
              ...
              (let [[resolved-as-namespace resolved-as-name lint-as?]
                ...
                (if-let [expanded (and transformed
                  ...
                  (let [unknown-ns? (= :clj-kondo/unknown-namespace resolved-namespace)
                        ...
                        analyzed
                        (analyze-known-call ctx expr children config ns-name lang base-lang resolved-as-clojure-var-name resolved-as-namespace resolved-as-name lint-as? resolved-namespace resolved-name top-level? defined-by defined-by->lint-as unresolved?)]
